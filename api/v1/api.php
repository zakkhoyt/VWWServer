...
//		$stmt->execute(array($userId, $token, $name, $code, $_SERVER['REMOTE_ADDR']));
		$stmt = $this->pdo->prepare('INSERT INTO users (first_name, last_name, email, passcode, uuid, created_at) VALUES (?, ?, ?, ?, ?, ?)');
		$stmt->execute(array("zakk", "hoyt", "zakkhoyt@gmail.com", "1111122222211111", "march 5th"));
...
//        echo "parsing command<br>";
	...
	function makePayload($senderName, $text)
	{
		...
		$payload = '{"aps":{"alert":"' . $nameJson . ': ' . $textJson . '","sound":"high_frequency.mp3"}}';
		return $payload;
	}
